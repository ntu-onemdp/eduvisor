...
class EnrolmentsModel:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.collection.create_index([("course_id", pymongo.ASCENDING), ("student_email", pymongo.ASCENDING)], unique=True)
    ...
    def add_enrollments(self, course_id, student_emails):
        ...
        try:
            ...
            enrollment_docs = [
                {"course_id": course_id, "student_email": f"{email.strip()}@e.ntu.edu.sg", "enrolled": True}
            ...
            insert_result = self.collection.bulk_write([
                pymongo.InsertOne(doc) for doc in enrollment_docs
            ], ordered=False)  

            return response_handler(200, f"Successfully enrolled students in {course_id}.")

        except pymongo.errors.BulkWriteError as e:
            inserted_count = e.details["nInserted"]
            return response_handler(200, f"Enrolled {inserted_count} students, as some were already enrolled.")
        
        except PyMongoError as e:
            return response_handler(500, "Database error while adding enrolments.", str(e))
    ...
    def check_enrollment(self, course_id, student_email):
        ...
        """
        try:
            result = self.collection.find_one({"course_id": course_id, "student_email": student_email})
            
            if result:
                return response_handler(200, f"Student {student_email} is enrolled in {course_id}.")
            else:
                return response_handler(404, f"Student {student_email} is NOT enrolled in {course_id}.")

        except PyMongoError as e:
            return response_handler(500, "Database error while checking enrollment.", str(e))
    ...
    def get_enrollments_by_user(self, user_email):
        ...
        try:
            ...
            enrollments_dict = {enrollment["course_id"]: enrollment for enrollment in enrollments}
            print(enrollments_dict)

            return response_handler(200, "Enrollments retrieved successfully.", data=enrollments_dict)

        except PyMongoError as e:
            return response_handler(500, "Database error while fetching enrollments.", str(e))
    ...
    def get_whitelisted_students(self, course_id):
        ...
        """
        try:
            students = list(self.collection.find({"course_id": course_id}, {"_id": 0, "student_email": 1}))
            return response_handler(200, "OK", [student["student_email"] for student in students])
        
        except PyMongoError as e:
            return response_handler(500, "Database error while fetching whitelisted students.", [])
    ...
    def remove_student_from_course(self, course_id, student_email):
        ...
        """
        try:
            result = self.collection.delete_one({"course_id": course_id, "student_email": student_email})
            
            if result.deleted_count > 0:
                return response_handler(200, f"Successfully removed {student_email} from {course_id}.")
            return response_handler(404, f"Student {student_email} is not enrolled in {course_id}.")
        
        except PyMongoError as e:
            return response_handler(500, "Database error while removing student.", str(e))