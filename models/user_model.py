...
def should_deactivate_account(registration_date):
    ...
    if datetime(reg_year, 5, 15) < registration_date <= datetime(reg_year, 12, 15): #case 1: student registered after 15 may to the 15 dec -> deactivate on 15 dec
...
class UserModel:
        ...
        try:
            ...
            if status == "ACTIVATED":
                ...
                if should_deactivate_account(registration_date):
                    self.collection.update_one({"user_id": oid}, {"$set": {"status": "DEACTIVATED"}})
                    return response_handler(403, "Account Deactivated")
    ...
    def increment_chat_id(self, user_id):
        try:
            result = self.collection.update_one({"user_id": user_id}, {"$inc": {"chat_id_counter": 1}})
