...
def main():
    ...
    st.markdown('<div class = "chattitle" >Ask me anything!</div>', unsafe_allow_html=True)
    ...
    if course_id:
        ...
        if vector_store_response["code"] == 200:  
            ...
            st.error('The chatbot is currently unavailable for use as no course files have been added.')
            return None
    else:
    ...
    if query : 
        if len(query)>MAX_CHAR_LIMIT: 
            st.error(f"Your query exceeds the maximum limit of characters. Please shorten your query.")
            query = None
        if total_tokens_used >= MAX_TOKEN_LIMIT: 
            st.error(f"You have reached the maximum usage limit for this chatbot. Please contact support.")
            query = None

    # 3b. process query
    if query:
        chat_controller.process_query(query,user_id,course_id,course_name,loaded_faiss_vs,llm)
