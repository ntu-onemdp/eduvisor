...
class StudentPortalView:
    ...
    def display_student_portal(self):
        ...
        st.warning('⚠︎ Students are not authorized to access this portal. If you are an instructor, please request access below.')
        ...
        if st.button("Request Access", type='primary'):
            if email and reason:
                create_request_response = self.controller.create_admin_request(self.user_id, email, reason)
                if create_request_response["code"] == 201: 
                    st.success('Your request has been sent and will be reviewed.')
                else: 
                    st.error(f"Failed to create request: {create_request_response['status']}")
            else:
